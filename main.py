...
if args.command == 'init':
    ...
    if not have_bucket:
        ...
        try:
            ...
            print(
                f"Don`t found bucket with name: {config['default']['bucket']}. Create new bucket with name {config['default']['bucket']}")
            sys.exit(0)
        except:
    ...
    for i in albums_list:
        ...
        with a.html(lang='ru'):
            with a.head():
                ...
                a.script(
                    src="https://cdnjs.cloudflare.com/ajax/libs/galleria/1.6.1/themes/classic/galleria.classic.min.js")
            with a.body():
                with a.div():
                    for my_bucket_object in my_bucket.objects.filter(Prefix=i.key):
                        ...
                        a.img(
                            src=f"{config['default']['endpoint_url']}/{my_bucket_object.bucket_name}/{my_bucket_object.key}",
                            alt=alt_t, width="960px", height="540px", background="#000")
                with a.p().small():
                    a("Вернуться на ")
